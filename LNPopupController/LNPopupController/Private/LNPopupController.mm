...
- (void)_presentPopupBarWithContentViewController:(UIViewController*)contentViewController openPopup:(BOOL)open animated:(BOOL)animated completion:(void(^)(void))completionBlock
{
	...
	if(_popupControllerTargetState == LNPopupPresentationStateBarHidden)
	{
		...
		if(@available(iOS 26, *))
		if(__LN_HAS_OS26_GLASS() && animated && self.popupBar.resolvedIsCustom == NO)
		{
			[UIView performWithoutAnimation:^{
		...
		if(animated && __LN_HAS_OS26_GLASS() && self.popupBar.resolvedIsCustom == NO)
		{
			...
			_runningBarAnimation = [[UIViewPropertyAnimator alloc] initWithDuration:LNPopupBarTransitionDuration dampingRatio:500 animations:^{
				...
				if(animated)
				{
					[UIView animateWithDuration:LNPopupBarTransitionDuration delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0 options:UIViewAnimationOptionAllowAnimatedContent animations:^{
						if(_containerController.shouldFadePopupBarOnDismiss && (!__LN_HAS_OS26_GLASS() || self.popupBar.resolvedIsCustom))
						{
