...
- (void)_layoutModernTabBarControllerFloatingPopupWithSuperFallback:(void(^)(void))superFallback API_AVAILABLE(ios(26.0))
{
	...
	{
		static Class tabBarContainerClass = NSClassFromString(LNPopupHiddenString("_UITabContainerView"));
		NSUInteger idx = [self.view.subviews indexOfObjectPassingTest:^BOOL(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
			return [obj isKindOfClass:tabBarContainerClass];
		}];
...
- (CGRect)defaultFrameForBottomDockingView
{
	...
	if(__LN_HAS_OS26_GLASS() && self._ln_isObjectFromSwiftUI)
	{
		//Apple is retarded. The toolbar is no longer part of the view hierarchy, and in SwiftUI, it isn't even sized correctly.
